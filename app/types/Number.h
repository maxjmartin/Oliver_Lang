...
namespace Oliver {
    ...
    number::number(std::string str) : _value(0.0, 0.0) {
        ...
        if (tokens.size()) {
            ...
            if (tokens.back().back() == 'i' || tokens.back().back() == 'j' || tokens.size() > 1) {
                ...
                imag = string_to_number<long long>(t, 10).value_or(0);
        ...
        if (tokens.size()) {
            ...
            real = string_to_number<long long>(tokens.front(), 10).value_or(0);
        }

        _value = num_type(real, imag);
    }

    number::number(int64_t value) : _value(static_cast<val_type>(value)) {
    }
    ...
    bool _is_(const number& self) {
        ...
        return (static_cast<bool>(self._value.real()) != 0 || static_cast<bool>(self._value.imag()) != 0);
    }
    ...
    order _comp_(const number& self, const var& other) {
        ...
        if (ptr) {
            if ((self.is_nan()) || static_cast<bool>(ptr->is_nan()) || static_cast<bool>(self._value.imag()) || static_cast<bool>(ptr->_value.imag())) {
    ...
    std::string _str_(const number& self, const Format_Args& fmt) {
        ...
        if (static_cast<bool>(imag) && !static_cast<bool>(real)) {

            return std::format("{}j", imag);
        }

        if (!static_cast<bool>(imag)) {

            return std::format("{}", real);
        }

        return std::format("({},{}j)", real, imag);
    }
    ...
    var _mod_(number& self, var& other) {
        ...
        if (ptr) {
            ...
            return number(static_cast<int64_t>(self._value.real()) % static_cast<int64_t>(ptr->_value.real()));
        }
        return var();
    }
    ...
    var _rem_(const number& self, var& other) {
        ...
        if (ptr) {
            ...
            if (static_cast<bool>(self.is_nan()) || static_cast<bool>(ptr->is_nan()) || static_cast<bool>(self._value.imag()) || static_cast<bool>(ptr->_value.imag())) {
                return number("nan");
            }
